...
## About The Project
Gatekeeper is a highly decoupled microservice created to authenticate users for my future apps.
...
- **Asynchronous Processing:** Redis Streams were used to send email for recovering a password, just in case the computer running Gatekeeper explodes and the process crashes.
- **Clean Architecture:** Instead of putting all the code in one place, I organized everything in Clean Architecture: Handler receives user requests, checking if the request is valid → Service handles the business' rules → Repository receives calls to actually change the data
- **gRPC Deep Dive:** gRPC is fast because of contracts you create with a .proto file. Here, you say what you want to receive and how the method should be like. gRPC itself receives binary data, so no json to slow the process down!
